...
### 문제 설게
1. 크기가 [N+1][C+1]인 배열 선언
2. 이중 반복문 안에서 점화식을 이용하여 배열에 값을 저장
    - memo[i][c]: 최대 비용 c인 상태에서 i번째 도시까지 투자했을 경우 최대 고객 수
    - memo[i][c] = Max(memo[i-1][c], memo[i-1][c - i번째 도시의 투자 비용에 대한 고객 수] + i번째 도시의 투자 비용)
3. 시간 복잡도는 N * C = 20 * 1000 = 2000 < 2 * 10^8 이므로 2초 안에 해결가능