...
2. DP로 해결하자
3. 크기가 [N+1][N*C+1]인 배열 선언
4. 이중 반복문 안에서 점화식을 이용하여 배열에 값을 저장
    - memo[i][c]: 최대 비용 c인 상태에서 i번째 앱까지 판단했을때의 최대 메모리
    - memo[i][c] = Max(memo[i-1][c], memo[i-1][c - i번째 앱의 비용] + i번째 앱의 메모리)
5. 시간 복잡도는 N * (N * C) = 100 * (100 * 100) = 10^6 < 10^8 이므로 1초 안에 해결가능